...
export default function Auth() {
  ...
  const handleLogin = async (e) => {
    ...
    if (!email.endsWith('@astronauts.id')) {
      setMessage('Error: Gunakan email @astronauts.id')
      setLoading(false)
      return
    }
  ...
  const handleSignUp = async () => {
    ...
    if (!email.endsWith('@astronauts.id')) {
      setMessage('Error: Hanya untuk email @astronauts.id')
      setLoading(false)
      return
    }
    ...
    if (error) setMessage(error.message)
    else setMessage('Cek email untuk konfirmasi pendaftaran!')
    setLoading(false)
  }
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input 
                type="email" placeholder="nama@astronauts.id" required
          ...
          {message && (
            <div className={`text-xs p-3 rounded-lg ${message.includes('Error') ? 'bg-red-500/10 text-red-400' : 'bg-green-500/10 text-green-400'}`}>
              {message}
            </div>
          )}
          ...
          <div className="flex flex-col gap-3 pt-2">
            <button 
              ...
              className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-2.5 rounded-lg transition-all flex items-center justify-center gap-2"
            >
